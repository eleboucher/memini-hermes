...
| `MEMINI_INJECT_RECALL_MIN_SCORE` | server default `0.5` (`0` pre-handshake) | composite floor (>=) on the final ranked score for auto-recall, sent as `min_rank_score` (server-enforced; applied client-side on pre-`min_rank_score` servers or values ≥ 1) |
